...
and \scalebox{1.2}{$\Omega = \frac{R_{box}}{L}$}. The reason for doing this instead of simply having  \scalebox{1.2}{$R_{box} = L$}, where $R_{box}$ is 
...
\end{equation}
From this (11.2) can be derived. It follows from this that as the size of our box increases, it would seem sensible for our stability plot to tend to (11.2). Figure 14 seems to show that this might be true for high gamma, but a more conclusive plot is definitely needed. There seems to be the opposite relationship for lower gamma, as can be shown by figure 16 which used \scalebox{1.1}{$R_{box}=20L$}, but this may just be a problem with the code. Over the next week or so we should be able to get some better plots, these are really just giving a rough idea of what it looks like.
